...
class DependencyPattern:
    ...
    capture_groups: list[dict[str, Any]] | None = None
...
class DependencyEngine:
    ...
    def _load_patterns(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Loaded {sum(len(p) for p in self.patterns.values())} dependency patterns"
            )

        except Exception as e:
            logger.error(f"Failed to load dependency patterns: {e}")

    def match_pattern(self, output: str) -> DependencyMatch | None:
        """
        ...
        for _category, patterns in self.patterns.items():
            for pattern in patterns:
                ...
                if match:
                    ...
                    if (
                        pattern.custom_handler
                        and pattern.custom_handler in self.custom_handlers
                    ):
                        handler = self.custom_handlers[pattern.custom_handler]
                        if "message_template" in handler:
                            handler_message = handler["message_template"].format(
                                **captured_data
                            )
    ...
    def suggest_fix(self, output: str) -> dict[str, Any] | None:
        """
    ...
    def apply_fix(
        self, fix_info: dict[str, Any], dry_run: bool = False
    ) -> tuple[bool, str]:
        """
        ...
        if action == "execute" and "fix_command" in fix_info:
            ...
            if result.return_code == 0:
                ...
                return (
                    False,
                    f"Failed to execute {fix_info['fix_command']}: {result.stderr}",
                )
    ...
    def test_pattern_matching(self, output: str) -> DependencyMatch | None:
        """
