...
class FormattingPatternEngine:
    ...
    def match_output(self, output: str) -> MatchResult | None:
        """
        ...
        for _tool_name, tool_patterns in self.patterns.items():
    ...
    def _try_pattern_match(
        self, output: str, pattern_config: dict[str, Any]
    ) -> MatchResult | None:
        """
        ...
        try:
            ...
            print(
                f"Warning: Invalid regex in pattern {pattern_config.get('id', 'unknown')}: {e}"
            )
    ...
    def match_specific_tool(self, output: str, tool: str) -> MatchResult | None:
        """
            ...
            for pattern_config in tool_patterns:
                ...
                for field in required_fields:
                    if field not in pattern_config:
                        errors.append(
                            f"Pattern {pattern_config.get('id', 'unknown')} missing field: {field}"
                        )
                ...
                try:
                    ...
                    errors.append(
                        f"Invalid regex in pattern {pattern_config.get('id', 'unknown')}: {e}"
                    )

                # Validate severity
                if pattern_config.get("severity") not in ["low", "medium", "high"]:
                    errors.append(
                        f"Invalid severity in pattern {pattern_config.get('id', 'unknown')}: {pattern_config.get('severity')}"
                    )
