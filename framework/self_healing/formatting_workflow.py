...
from .command_executor import SafeCommandExecutor, CommandResult, CommandExecutionError, CommandTimeoutError
from .syntax_verifier import SyntaxVerifierAndCommitter, GitCommitResult, SyntaxVerificationError, GitOperationError
...
class FormattingFixWorkflow:
    ...
    def process_tool_output(
        ...
        try:
            ...
            self.logger.info(f"Found fixable pattern: {match_result.pattern_id} for tool {match_result.tool}")
            ...
            if create_commit:
                ...
                if commit_result.success:
                    self.logger.info(f"Workflow completed successfully. Commit: {commit_result.commit_hash}")
                    return WorkflowResult(
                        success=True,
    ...
    def _verify_and_commit(self, match_result: MatchResult, verify_syntax: bool) -> GitCommitResult:
    ...
    def validate_configuration(self) -> List[str]:
        ...
        for tool in supported_tools:
            ...
            if not tool_info['available']:
                errors.append(f"Tool '{tool}' is not available: {tool_info.get('error', 'Unknown error')}")
        
        return errors
    
    def test_pattern_matching(self, test_output: str) -> Optional[MatchResult]:
        """
