...
class SyntaxVerifierAndCommitter:
    ...
    def get_changed_files(self) -> list[str]:
        ...
        try:
            ...
            raise GitOperationError(f"Failed to get git status: {e.stderr}") from e
    ...
    def check_python_syntax(self, file_path: str) -> SyntaxCheckResult:
        ...
        try:
            ...
            return SyntaxCheckResult(file_path=file_path, is_valid=True)
    ...
    def verify_changed_files_syntax(
        self, file_extensions: set[str] | None = None
        ...
        if failed_files:
            raise SyntaxVerificationError(
                f"Syntax errors found in {len(failed_files)} files", failed_files
            )
    ...
    def restore_changes(self) -> None:
        ...
        try:
            ...
            raise GitOperationError(f"Failed to restore changes: {e.stderr}") from e

    def stage_changes(self, files: list[str] | None = None) -> list[str]:
        """
        ...
        try:
            ...
            raise GitOperationError(f"Failed to stage changes: {e.stderr}") from e

    def create_commit(
        self, message: str, author: str | None = None, include_file_count: bool = True
    ) -> GitCommitResult:
        """
    ...
    def atomic_format_commit(
        ...
        try:
            ...
            self.restore_changes()
            raise GitOperationError(f"Atomic commit failed: {e}") from e
