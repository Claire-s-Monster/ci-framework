...
def formatting_config():
    ...
    config_path = Path(__file__).parent.parent.parent / "self_healing" / "formatting.yml"
...
def test_pattern_structure(formatting_config):
    ...
    required_fields = ["id", "name", "pattern", "description", "fix_command", 
                      "tool", "severity", "requires_git_commit", "commit_message_template"]
    
    for tool_name, tool_patterns in patterns.items():
        assert isinstance(tool_patterns, list), f"Patterns for {tool_name} should be a list"
    ...
    for tool_name, tool_patterns in patterns.items():
        for pattern in tool_patterns:
            ...
            try:
                ...
                pytest.fail(f"Invalid regex pattern '{regex_pattern}' in {pattern['id']}: {e}")
...
def test_black_pattern_matching():
    ...
    reformat_pattern = next(p for p in black_patterns if p["id"] == "black_would_reformat")
...
def test_engine_config_structure(formatting_config):
    ...
    required_fields = ["timeout_seconds", "max_file_size_mb", "backup_before_fix", 
                      "verify_syntax_after_fix", "allowed_file_extensions", 
                      "exclude_patterns"]
...
def test_git_config_structure(formatting_config):
    ...
    required_fields = ["create_commit", "commit_author", "commit_prefix", "include_file_count"]
...
def test_fix_commands_not_empty(formatting_config):
    ...
    for tool_name, tool_patterns in patterns.items():
        for pattern in tool_patterns:
            ...
            assert len(fix_command) > 5, f"Fix command too short in pattern {pattern['id']}"
...
def test_commit_message_templates_valid(formatting_config):
    ...
    commit_pattern = re.compile(r"^(feat|fix|docs|style|refactor|test|chore)(\(.+\))?: .+")
