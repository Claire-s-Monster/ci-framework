...
def test_create_commit_with_custom_author(mock_run, verifier):
    ...
    result = verifier.create_commit(
        "Test commit", author="Custom Author <custom@example.com>"
    )
...
def test_atomic_format_commit_success(mock_run, verifier, temp_dir):
    ...
    mock_run.side_effect = [
        MagicMock(
            stdout="M  test.py\n", returncode=0
        ),  # get_changed_files (verify_syntax)
        MagicMock(returncode=0),  # git add .
        MagicMock(
            stdout="M  test.py\n", returncode=0
        ),  # get_changed_files (stage_changes)
        MagicMock(stdout="test.py\n", returncode=0),  # git diff --cached --name-only
        MagicMock(
            stdout="[main abc123] fix(format): Auto-fix black violations (1 file)\n",
            returncode=0,
        ),  # git commit
    ]
...
def test_commit_message_template_substitution(mock_run, verifier, temp_dir):
    ...
    mock_run.side_effect = [
        ...
        MagicMock(stdout="test.py\n", returncode=0),  # git diff --cached --name-only
        MagicMock(
            stdout="[main abc123] fix(format): Auto-fix ruff violations at 2023-12-01 12:00:00 (1 file)\n",
            returncode=0,
        ),  # git commit
    ]
...
def test_verify_non_python_files_ignored(mock_run, verifier):
    ...
    mock_run.return_value = MagicMock(stdout="M  test.js\nM  test.txt\n", returncode=0)
...
def test_large_file_syntax_check(mock_run, verifier, temp_dir):
    ...
    large_content = "# Large file\n" + "\n".join(
        [f"var_{i} = {i}" for i in range(1000)]
    )
...
def test_create_commit_failure(mock_run, verifier):
    ...
    mock_run.side_effect = [
        ...
        subprocess.CalledProcessError(
            1, "git commit", stderr="Commit failed"
        ),  # git commit fails
    ]
