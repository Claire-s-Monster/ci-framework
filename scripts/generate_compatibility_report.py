...
class CompatibilityReportGenerator:
    ...
    def validate_ci_template(self) -> bool:
        ...
        try:
            ...
            matrix_valid = (
                python_versions == expected_python
                and os_platforms == expected_os
        ...
        for test_file in test_files:
            ...
            if not test_path.exists():
                self.report_data["validation_results"][
                    f"test_{test_file.split('/')[-1]}"
                ] = {"status": "skipped", "reason": f"Test file not found: {test_file}"}
                continue
            ...
            try:
                ...
                self.report_data["validation_results"][
                    f"test_{test_file.split('/')[-1]}"
                ] = {"status": "timeout", "reason": "Test execution timeout (300s)"}
                all_passed = False
            except Exception as e:
                self.report_data["validation_results"][
                    f"test_{test_file.split('/')[-1]}"
    ...
    def simulate_matrix_performance(self) -> bool:
        ...
        mock_performance = {
            ...
            "python_3.11_macos-latest": {
                "duration": 123.2,
                "memory_mb": 515,
                "cpu_percent": 47.3,
            },
            "python_3.12_ubuntu-latest": {
                "duration": 119.8,
                "memory_mb": 510,
                "cpu_percent": 45.0,
            },
            "python_3.12_macos-latest": {
                "duration": 124.1,
                "memory_mb": 518,
                "cpu_percent": 47.8,
            },
        ...
        variance_acceptable = all(
            var < 20.0 for var in [duration_variance, memory_variance, cpu_variance]
        )
    ...
    def generate_gate_checklist(self) -> dict[str, bool]:
        ...
        gates = {
            ...
            "performance_variance_under_20_percent": (
                validation_results.get("performance_simulation", {}).get("status")
                == "passed"
            ),
    ...
    def generate_recommendations(self) -> list[str]:
        ...
        if not recommendations:
            recommendations.append(
                "✅ All compatibility validations passed - matrix is ready for production"
            )
    ...
    def print_report(self):
        ...
        print(
            f"Platform: {self.report_data['generator_platform']['system']} {self.report_data['generator_platform']['platform']}"
        )
        print(f"Python: {self.report_data['generator_platform']['python_version']}")

        print(f"\n📊 OVERALL STATUS: {summary['overall_status']}")
        print(
            f"Gates Passed: {summary['gates_passed']}/{summary['total_gates']} ({summary['gate_success_rate']:.1f}%)"
        )
...
def main():
    ...
    # Save to file
    output_path = (
        project_root / "artifacts" / "reports" / "compatibility-matrix-report.json"
    )
