...
class PackageDetector:
    ...
    def detect_packages(self, package_types: set[str] = None) -> dict[str, list[dict]]:
        ...
        for root, dirs, _files in os.walk(self.root_dir):
    ...
    def _analyze_package(self, pkg_type: str, config_path: Path) -> dict | None:
        """
    ...
    def _analyze_pyproject_toml(self, pkg_type: str, config_path: Path) -> dict | None:
        ...
        try:
            ...
            if pkg_type == "pixi" and "pixi" in tool_section:
                project_info = tool_section["pixi"].get(
                    "workspace"
                ) or tool_section["pixi"].get("project", {})
                ...
                return self._create_package_info(
                    pkg_type, config_path, name, data=tool_section["poetry"]
                )
            ...
            with open(config_path) as f:
                ...
                if f"[tool.{pkg_type}" in content:
                    return self._create_package_info(
                        pkg_type, config_path, config_path.parent.name
                    )
    ...
    def _analyze_package_json(self, config_path: Path) -> dict:
        ...
        try:
            ...
            return self._create_package_info(
                "npm", config_path, config_path.parent.name
            )
    ...
    def _create_package_info(
        self, pkg_type: str, config_path: Path, name: str, data: dict = None
    ) -> dict:
    ...
    def _has_tests(self, package_dir: Path) -> bool:
        ...
        for _item in package_dir.rglob("test_*.py"):
            return True

        return False

    def _detect_commands(
        self, pkg_type: str, package_dir: Path, data: dict = None
    ) -> dict[str, str]:
